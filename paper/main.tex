...
\date{\small Technical report -- primary experiment and diagnostic ablation\\Version 1.2, September 16, 2026}
...
\begin{table*}[t]
\centering
\caption{Qwen2.5-Coder Table~5, independent EvalPlus leaderboard, and local primary pass@1. HE denotes HumanEval; HE+ denotes HumanEval+. Dashes indicate models absent from the current EvalPlus result data.}
\label{tab:results}
\scriptsize
...
The StarCoder2 discrepancy is not confined to the Qwen comparison table. The independent EvalPlus leaderboard reports the same 31.7\% HumanEval and 27.4\% HumanEval+ values, states that pass@1 uses greedy decoding, and distinguishes unmarked base models such as StarCoder2-3B as direct code completion~\citep{evalplus2026leaderboard,evalplus2026setup}. Our unmodified-EvalPlus result of 1.2\% therefore conflicts with a second party using the same evaluator family and nominal evaluation mode. That contradiction requires a pipeline-level explanation; agreement between our wrapper and our stock control alone does not resolve it.
...
\begin{table}[t]
\centering
\caption{StarCoder2-3B controls and post-hoc stop-rule ablation.}
\label{tab:ablation}
\scriptsize
...
Independent EvalPlus leaderboard & 31.7\% & 27.4\% \\
\bottomrule
\end{tabularx}
\end{table}

Removing the stop recovered 9.15~\pp{} on HumanEval and 7.93~\pp{} on HumanEval+ relative to the stock control, so the stop interaction is a demonstrated contributor. It did not approach the independent 31.7\% and 27.4\% values: gaps of 21.3 and 18.3 points remain. We therefore do not attribute the full discrepancy to this stop rule or to deployment sensitivity in general. Differences involving prompt construction, tokenizer special-token or beginning-of-sequence handling, or other undocumented pipeline details remain possible, but untested. The residual is reported as an unresolved anomaly.
...
\textbf{Single deployment stack.} We used one GPU, one precision, one inference library version, and one evaluator release. The result does not estimate variation across hardware, kernels, libraries, or quantization modes. It also does not support CPU performance claims.

\textbf{One greedy completion.} The primary endpoint measures deterministic greedy pass@1. It does not characterize sampling distributions or pass@5. The task bootstrap quantifies uncertainty across the finite task set; it is not a substitute for repeated stochastic completions.
...
\textbf{Diagnostic scope.} The StarCoder2 probe and stop-rule ablation were selected after observing the primary outcome and are explanatory rather than confirmatory. The exact stop change has a measured effect, but most of the independent-score gap remains unresolved. The data do not establish that StarCoder2 is generally inferior, that the independent score is false, or that the identified stop rule is the only relevant pipeline difference.
...
The artifact directory contains model-generated Python and should be treated as untrusted data. Re-evaluation should use the documented disposable container rather than executing samples on a host system. The code, protocol, retained artifacts, and report for the v1.2.0 release are archived on Zenodo under DOI \href{https://doi.org/10.5281/zenodo.22800651}{10.5281/zenodo.22800651}.

\section{Conclusion}

The published five-model HumanEval ordering did not survive our uniform local deployment condition. A single resolved reversal reduced Kendall's tau-b to 0.8 (task-bootstrap 95\% interval: 0.6 to 0.8) and met the preregistered failure-to-reproduce rule. The result was driven by StarCoder2-3B. Its 1.2\% unmodified-EvalPlus control confirms that the wrapper optimization did not cause the divergence, while the post-hoc 10.4\% stop ablation shows that the exact newline-\code{def} boundary caused a substantial fraction of the failures. It does not recover the independent 31.7\% result, leaving a 21.3-point HumanEval gap unresolved. Four other checkpoints were substantially closer to the displayed source scores, including a near-exact rounded match for Qwen2.5-Coder-3B.

The practical lesson is not that published tables are unusable. It is that their order should not be treated as checkpoint metadata. For base code models, the benchmark result is a property of the complete evaluation system. Reports should preserve prompt construction, decoding, stopping, sanitization, evaluator version, checkpoint revision, and task-level outputs together. The next diagnostic priority is to reproduce the independent EvalPlus StarCoder2 setup exactly enough to locate the residual gap; until then, the anomaly should not be generalized beyond the disclosed condition.
